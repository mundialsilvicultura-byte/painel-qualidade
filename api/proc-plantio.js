...
const PCFG = {dMin:40,dMax:44,pAnom:50,pDens:40,pParc:10};
function pNotaDens(m){
  if(m==null)return null;
  if(m>=PCFG.dMin&&m<=PCFG.dMax)return 100;
  const centro=(PCFG.dMin+PCFG.dMax)/2, passo=centro*0.05;
  const dist=(m<PCFG.dMin)?(PCFG.dMin-m):(m-PCFG.dMax);
...
// linhasFormularios: array de {linhas, ehQual} (principal + qual antiga)
// linhasParam: linhas da aba de parâmetros (compartilhada)
function processarPlantio(linhasPrincipal, linhasParam, linhasQual){
  ...
  for(const linhasF of abas){
    ...
    for(let r=2;r<linhasF.length;r++){
      ...
      const parcConf=validas.filter(d=>d>=PCFG.dMin&&d<=PCFG.dMax).length;
      ...
      const nDens=validas.length?pNotaDens(densMedia):null;
      let nFinal=null;
      if(nDens!=null){ const tp=PCFG.pAnom+PCFG.pDens+PCFG.pParc;
        nFinal=H.roundPy((PCFG.pAnom*nAnom+PCFG.pDens*nDens+PCFG.pParc*pctConf)/tp,1); }
      const parcelas=[];
