...
function processarReplantio(linhasPrincipal, linhasParam){
  ...
  for(let r=2;r<linhasParam.length;r++){
    ...
    linha.pctDesvio=H.num(row[mpar['Cópia % de Desvio']])||0;
  ...
  for(let r=2;r<linhasPrincipal.length;r++){
    ...
    const dvs=linhas.map(l=>l.pctDesvio).filter(x=>x!=null);
    const nc_mortas_pct=dvs.length?H.roundPy(dvs.reduce((a,b)=>a+b,0)/dvs.length*100,1):0;
